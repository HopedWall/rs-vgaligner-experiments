...
/usr/bin/time -o mapper_results.txt --verbose vgaligner map -i graph.idx -f reads_header.fa --also-align

# Convert my result to gam
vg convert graph.gfa -F reads_header-alignments.gaf > reads_header-alignments.gam

# Compare alignment
#vg gamcompare -r 100 -s reads_header-alignments.gam aln.gam 
